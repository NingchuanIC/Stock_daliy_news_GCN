...
class GCNNewsResidual(nn.Module):
	def __init__(
		self,
		price_dim: int = 15,
		news_dim: int = 6,
		...
		self.alpha = alpha
		self.gcn1 = GCNConv(price_dim, gcn_hidden)
		self.gcn2 = GCNConv(gcn_hidden, 1)
		self.news_mlp = nn.Sequential(
			nn.Linear(news_dim, news_hidden),
			nn.ReLU(),
			nn.Linear(news_hidden, 1),
		)

	def forward(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
		expected_dim = self.price_dim + self.news_dim
		if x.shape[1] != expected_dim:
			raise ValueError(
				f"Expected input feature dim={expected_dim}, got {x.shape[1]}."
			)

		price_x = x[:, : self.price_dim]
		news_x = x[:, self.price_dim :]

		h_price = self.gcn1(price_x, edge_index)
		h_price = torch.relu(h_price)
		pred_price = self.gcn2(h_price, edge_index)

		delta_news = self.news_mlp(news_x)

		return pred_price + self.alpha * delta_news
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		...
		type=float,
		default=0.0,
		help="Unused in residual model. Kept for backward CLI compatibility.",
	)
	parser.add_argument(
		"--alpha",
		type=float,
		default=0.1,
		help="Residual weight for news correction: pred = pred_price + alpha * delta_news.",
	)
	parser.add_argument(
	...
	parser.add_argument(
		"--save-model-path",
		type=Path,
		default=repo_root / "GCN_news_graph_feat_6_MLP" / "gcn_model.pt",
		help="Path to save trained model checkpoint.",
	)
	parser.add_argument(
		"--output-log",
		type=Path,
		default=repo_root / "GCN_news_graph_feat_6_MLP" / "gcn_train_log.txt",
...
def save_outputs(
	...
	ckpt = {
		...
		"model": "GCNNewsResidual(price:15->GCN[32,1], news:6->MLP[16,1], pred=price+alpha*news)",
	}
...
def main() -> None:
	...
	price_dim = 15
	news_dim = len(feature_cols) - price_dim
	if news_dim <= 0:
		raise ValueError(
			f"feature_cols length must be > {price_dim}, got {len(feature_cols)}"
		)

	model = GCNNewsResidual(
		price_dim=price_dim,
		news_dim=news_dim,
		gcn_hidden=32,
		news_hidden=16,
	...
	result_lines = [
		"GCN+News Residual Regression (fixed year split, prebuilt graph dataset)",
