...
\subsection{Residual News Fusion Ablation and Final Conclusion}
...
Therefore, for the main GNN result in this report, we keep \textbf{GCN + price-only + industry graph} as the strongest and cleanest baseline. The news-fusion experiments are treated as ablation evidence rather than as the final model.

This also clarifies the next research step: instead of further tuning node-level news fusion, the more meaningful direction is \textbf{GAT + price-only + industry graph}. Since vanilla GCN uses fixed normalized averaging, it cannot adaptively distinguish which neighbors are most informative. GAT introduces learnable attention weights over neighbors, making it a natural next architecture to test whether adaptive relation weighting can exceed the current price-only GCN baseline.

\subsection{Multi-Seed Robustness and Dropout Ablation (New Subsession)}

To provide a fair and unbiased summary for the GCN baseline, we run two additional controlled experiments on the same setup (industry graph, 15 price features, train 2019--2023, validation 2024, test 2025).

\paragraph{Experiment 1: Multi-seed robustness check (dropout $=0.0$).}
We rerun the price-only GCN with five random seeds. The main test metrics are:
\begin{itemize}
\item Test IC: $0.0422 \pm 0.0051$
\item Test RankIC: $0.0397 \pm 0.0015$
\item Top30 Sharpe: $2.53 \pm 0.85$
\item Top50 Sharpe: $2.73 \pm 0.89$
\end{itemize}

Compared with LightGBM (test IC $\approx 0.0497$), GCN is close but not stably superior: the best seed reaches about $0.0502$ (roughly on par with LightGBM), while the mean remains lower. Importantly, IC variance is relatively high, but RankIC is much more stable, indicating that GCN consistently captures cross-sectional ranking structure even when regression magnitude fluctuates.

\paragraph{Experiment 2: Dropout ablation (dropout $=0.3$, 5 seeds).}
We then test stronger regularization by setting dropout to 0.3 and repeating five-seed evaluation. The result is clearly negative:
\begin{itemize}
\item Test IC changes from $0.0422 \pm 0.0051$ to $0.0353 \pm 0.0141$
\item Mean IC decreases and variance increases
\end{itemize}

This indicates that the bottleneck is not classic overfitting. Instead, under weak and sparse financial signals, aggressive dropout further attenuates useful information and hurts stability.

\paragraph{One-sentence conclusion for GCN.}
GCN does not stably outperform LightGBM yet, but it is already close and consistently shows that graph structure contains usable predictive signal.

\paragraph{Implications for next-stage graph research.}
These two experiments motivate three concrete directions as the next stage:
\begin{itemize}
\item \textbf{Direction 1: GAT for adaptive relation weighting.} Learn neighbor importance instead of fixed averaging to improve stability and signal utilization.
\item \textbf{Direction 2: Rebuild graph structure.} Go beyond same-sector edges to richer, weighted, and potentially dynamic relations.
\item \textbf{Direction 3: Reconstruct news modeling.} Treat news as event-driven information (e.g., event nodes/edges, temporal gating, or late fusion) rather than direct dense node-feature concatenation.
\end{itemize}
